...
> **⚠️ Nota del proyecto (The Verb Project):** esta copia fue **podada a solo-documentación**.
> Se eliminaron `scripts/` y `agents/` para quitar código ejecutable de terceros, llamadas a la
> API de OpenAI y la capacidad de escribir hooks en `.claude/settings.json`.
> **No ejecutes ningún comando `node .../scripts/...` que aparezca abajo** — no existen.
> Ignorá el paso de bootstrap con `context.mjs` y usá directamente los documentos de `reference/`.
> Los comandos de navegador en vivo, generación de imágenes, hooks y pin/unpin no están disponibles.
