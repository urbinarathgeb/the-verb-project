...
## 3. Modo: Contrarreloj de Supervivencia ("Todo o Nada")
...
- **Ranking separado por modo** (Objetivo y Supervivencia tienen cada uno su propia tabla de posiciones). El Dojo no tiene ranking, solo progreso personal.
...
- `game_sessions` — un registro por partida completada: `user_id` (**NOT NULL**), `mode`, `level`, `time_ms`, `errors`, `verbs_matched`, `completed_at`. El `ritmo` del Modo Supervivencia (sección 3) se calcula a partir de `verbs_matched` y `time_ms` al momento de consultar el ranking — no requiere una columna adicional.
...
- `user_progress` — por usuario autenticado y verbo: aciertos, errores, última práctica (alimentado principalmente por el Dojo).
...
| --------- | ------------------------------------- | - | ----------- | ----------- | ------------ | ------- | ----------- | ---------- |
| `easy`   | `beginner` (49)                       | 6 | 16          | 90 s        | −2 s         | 5 s     | 3           | 5          |
| `medium`   | `beginner` + `intermediate` (86)      | 8 | 20          | 90 s        | −2 s         | 5 s     | 3           | 7          |
| `hard` | pool completo (106)                   | 10 | 24          | 100 s       | −3 s         | 5 s     | 3           | 9          |

- **Modo Supervivencia:** usa la misma `N` por nivel; el pool es el nivel completo.
- **Piso mínimo de aciertos para clasificar** en el ranking de Supervivencia: `MIN_MATCHES_FOR_RANKING = 5`.
...
- **El objetivo se dobló** al introducir la reposición diferida, que hizo el modo bastante más fácil. Son 5,6 s por acierto en `easy` y 4,2 s en `hard`. Se comprobó que la mecánica no lo limita: un bot que resuelve a la velocidad del DOM gana los tres niveles en 3-4 segundos, así que el único límite es la velocidad humana de reconocimiento — y eso sólo lo dice jugar.
